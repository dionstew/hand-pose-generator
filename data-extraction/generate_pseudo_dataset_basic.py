...
1. Membaca banyak gambar stereo dari satu folder
...
def rectify_stereo_image(
    ...
    return gray_left, gray_right, left_rectified_bgr, right_rectified_bgr, Q, P1
...
def process_image(image_path, calib, detector, args):
    gray_left, gray_right, left_rectified_bgr, right_rectified_bgr, Q, P1 = rectify_stereo_image(
    ...
    if args.save_vis:
        vis_dir = Path(args.output_dir) / "vis_2d"
        ...
        cv2.imwrite(str(vis_dir / f"{stem}_pseudo_pose_2d.png"), img_vis)
    ...
    if args.save_landmarks:
        lm_dir = Path(args.output_dir) / "landmarks"
...
def parse_args():
    ...
    parser.add_argument(
        "--input-dir",
        required=True,
        help="Folder berisi stereo image side-by-side."
    )
    ...
    parser.add_argument(
        "--save-vis",
        action="store_true",
        help="Simpan visualisasi 2D landmark + sumbu pose."
    )

    parser.add_argument(
        "--save-disparity",
        action="store_true",
        help="Simpan disparity visualization."
    )

    parser.add_argument(
        "--save-landmarks",
        action="store_true",
        help="Simpan landmark 2D/3D per gambar ke CSV."
    )
...
def main():
    ...
    summary_csv = output_dir / "pseudo_groundtruth_summary.csv"
