...
def rectify_stereo_image(
    ...
    map_right_x, map_right_y    = cv2.initUndistortRectifyMap(K_right,D_right,R2,P2,image_size_right,cv2.CV_32FC1)
    left_rectified_bgr          = cv2.remap(left_bgr,map_left_x,map_left_y,cv2.INTER_LINEAR)
    right_rectified_bgr         = cv2.remap(right_bgr,map_right_x,map_right_y,cv2.INTER_LINEAR)
...
def compute_hand_coordinate_frame(landmarks_3d, normal_facing_camera):
    """
    ...
    palm_center, z_axis = compute_palm_normal_from_landmarks(
        landmarks_3d,
        palm_indices=(0, 5, 9, 13, 17),
        normal_facing_camera=normal_facing_camera
    )
...
def normalize_quaternion_wxyz(q, eps=1e-8):
    """Normalisasi quaternion [w, x, y, z]."""
...
def fix_quaternion_sign_continuity(q, q_prev):
    """
    Quaternion q dan -q merepresentasikan rotasi sama.
    Fungsi ini mencegah tanda quaternion flip antar-frame.
    """
...
def smooth_quaternion_ema(q, q_prev_smooth, alpha=0.2):
    """
    Temporal smoothing quaternion dengan EMA sederhana.

    alpha kecil  -> lebih smooth tetapi lebih lambat mengikuti gerakan.
    alpha besar  -> lebih responsif tetapi lebih jitter.
    """
    q = normalize_quaternion_wxyz(q)

    if q is None:
        return q_prev_smooth

    if q_prev_smooth is None:
        return q

    alpha = float(np.clip(alpha, 0.0, 1.0))
    q = fix_quaternion_sign_continuity(q, q_prev_smooth)

    q_smooth = (1.0 - alpha) * np.asarray(q_prev_smooth, dtype=np.float32) + alpha * q
    q_smooth = normalize_quaternion_wxyz(q_smooth)
...
def quaternion_wxyz_to_rotation_matrix(q):
    """Konversi quaternion [w, x, y, z] ke rotation matrix 3x3."""
    q = normalize_quaternion_wxyz(q)

    if q is None:
        raise ValueError("Quaternion tidak valid untuk dikonversi ke rotation matrix.")
...
def fix_rotation_axis_continuity(R, R_prev):
    """
    Mencegah sumbu lokal tangan flip antar-frame.

    R kolom:
        kolom 0 = X_hand
        kolom 1 = Y_hand
        kolom 2 = Z_hand / palm normal
    """
    ...
    R_prev = np.asarray(R_prev, dtype=np.float32)

    # Cegah palm normal/Z tiba-tiba berlawanan arah.
    if np.dot(R[:, 2], R_prev[:, 2]) < 0:
        R[:, 2] *= -1.0
        R[:, 0] *= -1.0

    # Cegah Y axis tiba-tiba berlawanan arah.
    if np.dot(R[:, 1], R_prev[:, 1]) < 0:
        R[:, 1] *= -1.0
        R[:, 0] *= -1.0

    # Orthonormalisasi ulang ringan agar tetap valid sebagai rotation matrix.
    x_axis = normalize_vector(R[:, 0])
    y_axis = normalize_vector(R[:, 1])
    z_axis = normalize_vector(R[:, 2])

    if x_axis is None or y_axis is None or z_axis is None:
        return R

    # Pertahankan definisi right-handed: X = Y x Z, Y = Z x X.
...
def smooth_translation_ema(t, t_prev_smooth, alpha=0.2):
    """EMA untuk translation. Dipakai hanya jika --smooth-translation aktif."""
    t = np.asarray(t, dtype=np.float32).reshape(3)

    if t_prev_smooth is None:
        return t

    alpha = float(np.clip(alpha, 0.0, 1.0))
    return ((1.0 - alpha) * np.asarray(t_prev_smooth, dtype=np.float32) + alpha * t).astype(np.float32)


class PoseTemporalState:
    """State temporal untuk satu sequence/video/folder."""
    def __init__(self):
        self.prev_R_smooth = None
        self.prev_q_smooth = None
        self.prev_t_smooth = None


def apply_temporal_pose_smoothing(R_cam_hand, t_cam_hand, q_wxyz, state, args):
    """
    Apply continuity correction + quaternion EMA.

    Return:
        R_out, t_out, q_out
    """
    if state is None or not getattr(args, "temporal_smoothing", True):
        return R_cam_hand, t_cam_hand, q_wxyz

    R_cont = fix_rotation_axis_continuity(R_cam_hand, state.prev_R_smooth)
    q_cont = rotation_matrix_to_quaternion_wxyz(R_cont)

    q_smooth = smooth_quaternion_ema(
        q=q_cont,
        q_prev_smooth=state.prev_q_smooth,
        alpha=getattr(args, "rotation_smoothing_alpha", 0.2),
    )

    if q_smooth is None:
        q_smooth = q_cont

    R_smooth = quaternion_wxyz_to_rotation_matrix(q_smooth)

    if getattr(args, "smooth_translation", False):
        t_smooth = smooth_translation_ema(
            t_cam_hand,
            state.prev_t_smooth,
            alpha=getattr(args, "translation_smoothing_alpha", 0.2),
        )
    else:
        t_smooth = np.asarray(t_cam_hand, dtype=np.float32).reshape(3)

    state.prev_R_smooth = R_smooth.copy()
    state.prev_q_smooth = q_smooth.copy()
    state.prev_t_smooth = t_smooth.copy()

    return R_smooth.astype(np.float32), t_smooth.astype(np.float32), q_smooth.astype(np.float32)


def make_sequence_key(image_path, input_root, reset_by_parent=True):
    """
    Key untuk reset smoothing per sequence.
    Default: parent folder relatif terhadap input_root dianggap satu video/sequence.
    """
    if not reset_by_parent:
        return "__global__"
...
def process_image(image_path, calib, detector, args, pose_state=None):
    ...
    R_cam_hand, t_cam_hand = compute_hand_coordinate_frame(landmarks_3d, args.normal_facing_camera)
    q_wxyz = rotation_matrix_to_quaternion_wxyz(R_cam_hand)

    # Simpan raw pose untuk debugging, lalu pakai pose yang sudah distabilkan
    # sebagai pseudo-label utama jika temporal smoothing aktif.
    R_raw = R_cam_hand.copy()
    t_raw = t_cam_hand.copy()
    q_raw = q_wxyz.copy()

    R_cam_hand, t_cam_hand, q_wxyz = apply_temporal_pose_smoothing(
    ...
    record = {
        ...
        "sequence_key": make_sequence_key(
            image_path,
            args.input_dir,
            reset_by_parent=args.reset_smoothing_by_parent,
        ),
        "status": "OK",
        "valid_landmarks_3d": valid_landmarks,
        "temporal_smoothing": bool(args.temporal_smoothing),

        # Smoothed/final pseudo-label. Kolom lama dipertahankan agar kompatibel
        # dengan script training yang sudah ada.
        ...
        # Raw pose sebelum smoothing untuk audit/debug.
        "raw_Tx": float(t_raw[0]),
        "raw_Ty": float(t_raw[1]),
        "raw_Tz": float(t_raw[2]),
        "raw_qw": float(q_raw[0]),
        "raw_qx": float(q_raw[1]),
        "raw_qy": float(q_raw[2]),
        "raw_qz": float(q_raw[3]),
    }
...
def main():
    ...
    print(f"Temporal smoothing rotasi : {args.temporal_smoothing}")
    print(f"Rotation smoothing alpha  : {args.rotation_smoothing_alpha}")
    print(f"Translation smoothing     : {args.smooth_translation}")
    print(f"Reset smoothing by parent : {args.reset_smoothing_by_parent}")
    ...
    try:
        for idx, image_path in enumerate(images, start=1, ):
            ...
            try:
                ...
                pose_state = pose_states.setdefault(sequence_key, PoseTemporalState())
